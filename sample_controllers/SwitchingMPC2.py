...
class SMPC2(Controller):
    ...
    def compute_next_command(self, timestep, state, debug=False):
        ...
        for k in range(self.N):
            ...
            for c in range(self.num_cells):
                if congestion_state[c] == 1:
                    # if congested, constrain by supply, penalize distance from x_lower in objective
                    if c != self.num_cells - 1:
                        constraints += [f[c, k] <= x[c+1, k] * self.w_list[c+1] + self.supply_b_list[c+1]]
                        objective += 50 * sum(abs(x[c, k] - (self.x_lower_list[c] - self.low_buffer))) * (1 - self.beta_list[c])
                    else:
                        objective += 50 * sum(abs(x[c, k] - (self.x_lower_list[c] - self.low_buffer)))
                else:
                    # if not, only constrain by demand, and penalize dist from x_upper in objective
                    if c != self.num_cells - 1:
                        objective += sum(abs(x[c, k] - (self.x_upper_list[c] - self.up_buffer))) * (1 - self.beta_list[c])
                    else:
                        objective += sum(abs(x[c, k] - (self.x_upper_list[c] - self.up_buffer)))
        ...
        command = np.expand_dims(u.value[:, 0], axis=0).transpose() # return commands for all onramps
